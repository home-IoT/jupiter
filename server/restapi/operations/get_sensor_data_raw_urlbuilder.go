...
func (o *GetSensorDataRawURL) Build() (*url.URL, error) {
	...
	var _path = "/sensors/{sensorId}/raw"

	sensorID := o.SensorID
	if sensorID != "" {
		_path = strings.Replace(_path, "{sensorId}", sensorID, -1)
	} else {
		return nil, errors.New("SensorID is required on GetSensorDataRawURL")
	}
...
func (o *GetSensorDataRawURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetSensorDataRawURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetSensorDataRawURL")
	}
